...
CPlayer::CPlayer() 
	...
	m_bIsBuffActive(false), m_bCanShoot(false), m_iLastDir(DIR_RIGHT), m_bStep_Block(false), m_iLife(0), m_bPlay(true), m_fPTime(0.f)
{
...
{
	//?Œë ˆ?´ì–´ ?¤ëª¨???¬ê¸°ë¥??•í•˜??ê²?
	m_tInfo = { 400.f, 300.f, 30.f, 30.f }; //xì¢Œí‘œ, yì¢Œí‘œ, ê°€ë¡œê¸¸?? ?¸ë¡œê¸¸ì´
	m_fSpeed = 5.f;
	m_fJumpPower = 15.f;
	m_fkg = 9.8f;
...
void CPlayer::Jumping(void)
{
	if (m_bPlay)
	{
		...
		if (m_bStep_Monster)
		{
			...
			if (b_LineCol && m_tInfo.fY > fY) //ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½Ï±ï¿½
			{
				m_fTime = 0.0f;
			}
		else if (m_bJump)
